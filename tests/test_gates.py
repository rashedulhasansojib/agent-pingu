...
`loop/SKILL.md` claims "the gates are what make autonomy safe". Until now that
...
def test_every_phase_in_the_gate_table_has_a_declared_gate():
    """The table in loop/SKILL.md and GATES are the same list written twice."""
    assert set(loop.GATES) == {
...
def test_a_manual_check_never_passes_even_when_executing(ready_vault):
    ...
    result = loop.run_gate(ready_vault, "diagnose", execute=True)
...
def test_setup_gate_fails_while_notes_are_still_templates(vault):
    result = loop.run_gate(vault, "setup")
...
def test_setup_gate_passes_once_the_templates_are_filled(ready_vault):
    result = loop.run_gate(ready_vault, "setup")
...
def test_talk_gate_fails_when_non_goals_is_an_empty_heading(ready_vault):
    ...
    result = loop.run_gate(ready_vault, "talk")
...
def test_talk_gate_passes_when_both_sections_have_content(ready_vault):
    ...
    assert loop.run_gate(ready_vault, "talk")["ok"] is True


def test_talk_gate_fails_when_there_is_no_brief_at_all(ready_vault):
    assert loop.run_gate(ready_vault, "talk")["ok"] is False
...
def test_plan_gate_fails_when_a_task_has_no_acceptance_criteria(ready_vault):
    ...
    result = loop.run_gate(ready_vault, "plan")
...
def test_plan_gate_passes_when_every_task_has_a_checkbox_criterion(ready_vault):
    ...
    assert loop.run_gate(ready_vault, "plan")["ok"] is True
...
def test_a_command_gate_is_planned_not_run_by_default(ready_vault):
    ...
    result = loop.run_gate(ready_vault, "verify")
...
def test_a_command_gate_runs_and_passes_when_executed(ready_vault):
    ...
    result = loop.run_gate(ready_vault, "verify", execute=True)
...
def test_a_command_gate_never_uses_a_shell(ready_vault, tmp_path):
    ...
    loop.run_gate(ready_vault, "verify", execute=True)
...
def test_gate_cli_exits_zero_when_nothing_it_can_check_is_broken(run_loop, ready_vault, capsys):
    assert run_loop("gate", "setup") == 0
    assert "passed" in capsys.readouterr().out


def test_gate_cli_exits_one_on_a_real_failure(run_loop, vault, capsys):
    assert run_loop("gate", "setup") == 1
    assert "failed" in capsys.readouterr().out


def test_gate_cli_defaults_to_the_phase_the_vault_is_in(run_loop, ready_vault, capsys):
    """`loop gate` with no argument gates whatever infer_phase reports, so the
    ...
    assert run_loop("gate") == 1
    assert "[gate] execute" in capsys.readouterr().out


def test_gate_cli_says_so_when_the_loop_is_closed(run_loop, ready_vault, capsys):
    """infer_phase can report `done`, which has no gate. That is not an error."""
    write_note(ready_vault, "tasks/T-0001-x.md", type="task", id="T-0001",
               status="done", work_type="chore", title="bump", epic="")

    assert run_loop("gate") == 0
    assert "nothing left to gate" in capsys.readouterr().out


def test_gate_cli_rejects_an_unknown_phase(run_loop, ready_vault):
    assert run_loop("gate", "nonsense") == 1


def test_gate_cli_reports_manual_checks_visibly(run_loop, ready_vault, capsys):
    run_loop("gate", "diagnose", "--execute")
...
def test_vault_init_seeds_the_command_keys(vault):
    ...
    frontmatter = loop.parse_frontmatter(vault / "context.md")

    assert "test_command" in frontmatter[loop.DECLARED]
    assert "lint_command" in frontmatter[loop.DECLARED]
...
def test_the_seeded_command_is_empty_not_a_guess(vault):
    ...
    argv, error = loop.declared_command(vault, "test_command")
