...
`pingu status` reports the autonomy level for this install on its `[pingu] autonomy:` line. On `full-loop` run every phase of the lane to the end and stop once, with a review packet. On `gated` stop after each phase and wait. Read the level from that line — never assume it.
